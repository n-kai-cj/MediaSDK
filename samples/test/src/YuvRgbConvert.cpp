...
// USE IPP
// --------------------------------------------------------------------------
void YuvRgbConvert::nv12_rgb24_ipp(unsigned char I[],
	...
	unsigned char J[])
{
	...
	// initialize IPP with optimized layer
	ipp_status = ippInit();
	if (ipp_status != ippStsNoErr)
	{
		fprintf(stderr, "IppInit() Error:\n");
		fprintf(stderr, "%s\n", (char*)ippGetStatusString(ipp_status));
		return;
	}

	const Ipp8u* pSrcY = (Ipp8u*)I;
	int srcYStep = image_width;
	Ipp8u* pSrcCbCr = (Ipp8u*)&I[image_width * image_height];
	int srcCbCrStep = image_width;
	Ipp8u* pDst = (Ipp8u*)J;
	int dstStep = image_width * 3;
	IppiSize roiSize = { image_width, image_height };

	ipp_status = ippiYCbCr420ToRGB_8u_P2C3R(pSrcY, srcYStep, pSrcCbCr, srcCbCrStep, pDst, dstStep, roiSize);

}

void YuvRgbConvert::nv21_rgb24_ipp(unsigned char I[],
	const int image_width,
	const int image_height,
	unsigned char J[])
{
	...
	ipp_status = ippiYCrCb420ToRGB_8u_P2C3R(pSrcY, srcYStep, pSrcCbCr, srcCbCrStep, pDst, dstStep, roiSize);
...
public:
	...
	RGB2YUVParam(double Rf, double Bf, double YMin, double YMax, double CbCrRange)
	{
		this->r_factor = FIXED_POINT_VALUE(Rf, 8);
		this->g_factor = 256 - FIXED_POINT_VALUE(Rf, 8) - FIXED_POINT_VALUE(Bf, 8);
		this->b_factor = FIXED_POINT_VALUE(Bf, 8);
		this->cb_factor = FIXED_POINT_VALUE((CbCrRange / 255.0) / (2.0 * (1 - Bf)), 8);
		this->cr_factor = FIXED_POINT_VALUE((CbCrRange / 255.0) / (2.0 * (1 - Rf)), 8);
		this->y_factor = FIXED_POINT_VALUE((YMax - YMin) / 255.0, 7);
		this->y_offset = YMin;
	}
};
...
public:
	...
	uint8_t y_factor;    // [(YMax-YMin)/255]
	uint8_t y_offset;    // YMin
	YUV2RGBParam(double Rf, double Bf, double YMin, double YMax, double CbCrRange)
	{
		this->cb_factor = FIXED_POINT_VALUE(255.0 * (2.0 * (1 - Bf)) / CbCrRange, 6);
		this->cr_factor = FIXED_POINT_VALUE(255.0 * (2.0 * (1 - Rf)) / CbCrRange, 6);
		this->g_cb_factor = FIXED_POINT_VALUE(Bf / (1.0 - Bf - Rf) * 255.0 * (2.0 * (1 - Bf)) / CbCrRange, 7);
		this->g_cr_factor = FIXED_POINT_VALUE(Rf / (1.0 - Bf - Rf) * 255.0 * (2.0 * (1 - Rf)) / CbCrRange, 7);
		this->y_factor = FIXED_POINT_VALUE(255.0 / (YMax - YMin), 7);
		this->y_offset = YMin;
	}
};
...
static const RGB2YUVParam RGB2YUV[3] = {
	// ITU-T T.871 (JPEG)
	RGB2YUVParam(0.299, 0.114, 0.0, 255.0, 255.0),
	// ITU-R BT.601-7
	RGB2YUVParam(0.299, 0.114, 16.0, 235.0, 224.0),
	// ITU-R BT.709-6
	RGB2YUVParam(0.2126, 0.0722, 16.0, 235.0, 224.0)
};

static const YUV2RGBParam YUV2RGB[3] = {
	// ITU-T T.871 (JPEG)
	YUV2RGBParam(0.299, 0.114, 0.0, 255.0, 255.0),
	// ITU-R BT.601-7
	YUV2RGBParam(0.299, 0.114, 16.0, 235.0, 224.0),
	// ITU-R BT.709-6
	YUV2RGBParam(0.2126, 0.0722, 16.0, 235.0, 224.0)
};


void YuvRgbConvert::rgb24_yuv420_std(
...
void YuvRgbConvert::rgb32_yuv420_std(
	uint32_t width, uint32_t height, 
...
void YuvRgbConvert::yuv420_rgb24_std(
	uint32_t width, uint32_t height, 
...
void YuvRgbConvert::nv12_rgb24_std(
	uint32_t width, uint32_t height, 
...
void YuvRgbConvert::nv21_rgb24_std(
	uint32_t width, uint32_t height, 
	const uint8_t *Y, const uint8_t *UV, uint32_t Y_stride, uint32_t UV_stride, 
	uint8_t *RGB, uint32_t RGB_stride, 
	YCbCrType yuv_type)
...
void YuvRgbConvert::rgb24_yuv420_sse(uint32_t width, uint32_t height, 
	const uint8_t *RGB, uint32_t RGB_stride, 
	uint8_t *Y, uint8_t *U, uint8_t *V, uint32_t Y_stride, uint32_t UV_stride, 
	YCbCrType yuv_type)
{
...
void YuvRgbConvert::rgb24_yuv420_sseu(uint32_t width, uint32_t height,
	const uint8_t *RGB, uint32_t RGB_stride, 
	uint8_t *Y, uint8_t *U, uint8_t *V, uint32_t Y_stride, uint32_t UV_stride, 
	YCbCrType yuv_type)
{
...
void YuvRgbConvert::rgb32_yuv420_sse(uint32_t width, uint32_t height,
	const uint8_t *RGBA, uint32_t RGBA_stride, 
	uint8_t *Y, uint8_t *U, uint8_t *V, uint32_t Y_stride, uint32_t UV_stride, 
	YCbCrType yuv_type)
{
...
void YuvRgbConvert::rgb32_yuv420_sseu(uint32_t width, uint32_t height,
	const uint8_t *RGBA, uint32_t RGBA_stride, 
	uint8_t *Y, uint8_t *U, uint8_t *V, uint32_t Y_stride, uint32_t UV_stride, 
	YCbCrType yuv_type)
{
...
void YuvRgbConvert::yuv420_rgb24_sse(
	uint32_t width, uint32_t height, 
	const uint8_t *Y, const uint8_t *U, const uint8_t *V, uint32_t Y_stride, uint32_t UV_stride, 
	uint8_t *RGB, uint32_t RGB_stride, 
	YCbCrType yuv_type)
...
void YuvRgbConvert::yuv420_rgb24_sseu(
	uint32_t width, uint32_t height, 
	const uint8_t *Y, const uint8_t *U, const uint8_t *V, uint32_t Y_stride, uint32_t UV_stride, 
	uint8_t *RGB, uint32_t RGB_stride, 
	YCbCrType yuv_type)
...
void YuvRgbConvert::nv12_rgb24_sse(
	uint32_t width, uint32_t height, 
	const uint8_t *Y, const uint8_t *UV, uint32_t Y_stride, uint32_t UV_stride, 
	uint8_t *RGB, uint32_t RGB_stride, 
	YCbCrType yuv_type)
...
void YuvRgbConvert::nv12_rgb24_sseu(
	uint32_t width, uint32_t height, 
	const uint8_t *Y, const uint8_t *UV, uint32_t Y_stride, uint32_t UV_stride, 
	uint8_t *RGB, uint32_t RGB_stride, 
	YCbCrType yuv_type)
...
void YuvRgbConvert::nv21_rgb24_sse(
	uint32_t width, uint32_t height, 
	const uint8_t *Y, const uint8_t *UV, uint32_t Y_stride, uint32_t UV_stride, 
	uint8_t *RGB, uint32_t RGB_stride, 
	YCbCrType yuv_type)
...
void YuvRgbConvert::nv21_rgb24_sseu(
	uint32_t width, uint32_t height, 
	const uint8_t *Y, const uint8_t *UV, uint32_t Y_stride, uint32_t UV_stride, 
	uint8_t *RGB, uint32_t RGB_stride, 
	YCbCrType yuv_type)
